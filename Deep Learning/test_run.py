...
def main(paramModelName, paramArrayLen):

    print(paramModelName,paramArrayLen)
    checkpoint_path = './tmp/finished/'
    model_name = paramModelName+'_10000.pkl'
    TEST_ARRAY_LENGTH = paramArrayLen
    NUM_OF_TESTS = 10000
    ...
    processor_factory = clrs.get_processor_factory(paramModelName, use_ln=True)
    model_params = dict(
    ...
    accuracies = test_model(rng_key, model, test_sampler, num_tests=NUM_OF_TESTS)
...
def test_model(rng_key, model, sampler, num_tests):
    accuracies = []
